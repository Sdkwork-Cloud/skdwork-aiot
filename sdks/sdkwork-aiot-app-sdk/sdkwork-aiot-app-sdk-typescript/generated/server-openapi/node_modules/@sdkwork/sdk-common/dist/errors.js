...
var SdkError = class extends Error {
	...
	static fromApiResult(result, httpStatus) {
		...
		switch (code) {
			...
			default:
				if (code.startsWith("5")) return new ServerError(message, httpStatus ?? HTTP_STATUS.INTERNAL_SERVER_ERROR);
	...
	static fromHttpStatus(status, message) {
		...
		switch (status) {
			...
			case HTTP_STATUS.UNPROCESSABLE_ENTITY: return new ValidationError(defaultMessage);
			case HTTP_STATUS.UNAUTHORIZED: return new AuthenticationError(defaultMessage);
			case HTTP_STATUS.FORBIDDEN: return new ForbiddenError(defaultMessage);
			case HTTP_STATUS.NOT_FOUND: return new NotFoundError(defaultMessage);
			case HTTP_STATUS.METHOD_NOT_ALLOWED: return new ValidationError(defaultMessage);
			case HTTP_STATUS.CONFLICT: return new ConflictError(defaultMessage);
			case HTTP_STATUS.TOO_MANY_REQUESTS: return new RateLimitError(defaultMessage);
			case HTTP_STATUS.INTERNAL_SERVER_ERROR: return new ServerError(defaultMessage, status);
			case HTTP_STATUS.BAD_GATEWAY: return new BadGatewayError(defaultMessage);
			case HTTP_STATUS.SERVICE_UNAVAILABLE: return new ServiceUnavailableError(defaultMessage);
			case HTTP_STATUS.GATEWAY_TIMEOUT: return new GatewayTimeoutError(defaultMessage);
			default:
...
//#endregion
export { AuthenticationError, BadGatewayError, BusinessError, CancelledError, ConflictError, ForbiddenError, GatewayTimeoutError, MethodNotAllowedError, NetworkError, NotFoundError, RateLimitError, SdkError, ServerError, ServiceUnavailableError, TimeoutError, TokenExpiredError, TokenInvalidError, ValidationError, isAuthError, isBusinessError, isCancelledError, isNetworkError, isRateLimitError, isRetryableError, isSdkError, isServerError, isTimeoutError, isValidationError };
